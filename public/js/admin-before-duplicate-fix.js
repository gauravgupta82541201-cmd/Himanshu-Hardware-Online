...
function updateInventoryStats() {
  ...
  const out = adminProducts.filter(p => {
    return Number(p.stock || 0) <= 0 || p.available === false;
  }).length;
  ...
  if (alert) {
    if (lowStock > 0 || out > 0) {
      ...
      alert.innerHTML =
        `⚠️ <b>Inventory Alert:</b> ${lowStock} low-stock aur ${out} out-of-stock product hain.`;
    } else {
...
function getFilteredProducts() {
  ...
  const availability = $("inventoryAvailability")?.value || "";
